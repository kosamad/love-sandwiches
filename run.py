...
def caluculate_stock_data(data):
    ...
    new_stock_data = []
    for column in data: # loops through each column in our data
        int_column = [int(num) for num in column] 
        average = sum(int_column) / len(int_column) # getting averages from each column
        stock_num = average * 1.1 # adds 10%
        new_stock_data.append(round(stock_num)) #rounds to whole number
